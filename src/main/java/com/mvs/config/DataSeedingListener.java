...
public class DataSeedingListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    void AddNewClubAndPlayer(String url) {
        try {
            ...
            ClubInfo clubInfo = clubService.findByName(nameClub);
            if (clubInfo == null) {
                clubInfo = new ClubInfo();
                clubInfo.setImage(element.select("body > div.container.main > div:nth-child(3) > div.col-md-8 > div:nth-child(3) > div:nth-child(2) > div > div.panel-heading > h3 > a:nth-child(1) > img").attr("abs:src"));
                clubInfo.setName(nameClub);
                //error with saveorUpdate
                clubService.addClub(clubInfo);
            }
            club = ConvertBeanAndModel.convertClubBeanToModel(clubService.findByName(clubInfo.getName()));
            ...
            playerService.addPlayer(ConvertBeanAndModel.convertPlayerModeltoBean(player));
